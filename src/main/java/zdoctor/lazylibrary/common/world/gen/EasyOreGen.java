...
public class EasyOreGen extends WorldGenMinable implements IEasyWorldGenerator {
	...
	public static final Predicate<IBlockState> ALL_COMMON = combine(DIRT, STONE, GRAVEL, SAND, SANDSTONE, NETHERRACK,
			SOULSAND);
	...
	public EasyOreGen(IBlockState state, int blockCount, Predicate<IBlockState> genPredicate) {
		super(state, blockCount, t -> genPredicate.test(t));
	...
	/**
	 * Used to set the levels the ore can be found in
	 * 
	 * @param min
	 * @param max
	 * @return
		...
		if (getCustomWorldGenerator() != null) {
			...
			if (genInWorlds.size() > 0) {
				if (genInWorlds.contains(world.provider.getDimensionType())) {
					BlockPos position = new BlockPos(chunkX * 16 + random.nextInt(16), getRandomY(world.rand),
	...
	public static Predicate<IBlockState> combine(Predicate<IBlockState>... genPredicate) {
		return t -> {
			for (Predicate<IBlockState> predicate : genPredicate)
				if (predicate.test(t))
					return true;
			return false;
		};
